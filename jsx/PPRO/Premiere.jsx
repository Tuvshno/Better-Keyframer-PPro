...
$._PPP_={
	...
	keepPanelLoaded : function () {
		app.setExtensionPersistent("com.tuvshno.KeyKeyPalmer", 0); // 0, while testing (to enable rapid reload); 1 for "Never unload me, even when not visible."
	},
	...
	myActiveSequenceSelectionChangedFxn : function () {
		...
		if (seq) {
			...
			if (sel) {
				...
				// Send first selection to plugin panel
				var selectedClip = sel[0];
				var components = [];

				for (var i = 0; i < selectedClip.components.length; i++) {
					var component = selectedClip.components[i];
					var properties = [];
					...
					var eventObj = new ExternalObject("lib:PlugPlugExternalObject");
				...
				// Update event for each selection
...
function simpleSerialize(obj) {
	...
	// Use standard JavaScript checks instead of Array.isArray and Object.keys
	if (Object.prototype.toString.call(obj) === '[object Array]') {
			serializedString += '[';
			for (var i = 0; i < obj.length; i++) {
					serializedString += simpleSerialize(obj[i]);
					if (i < obj.length - 1) serializedString += '|'; // Separator for array items
			}
			serializedString += ']';
			...
			for (var key in obj) {
					if (obj.hasOwnProperty(key)) {
							serializedString += key + ':' + simpleSerialize(obj[key]);
							serializedString += ',';
					}
			}
			serializedString = serializedString.replace(/,$/, ''); // Remove trailing comma
			serializedString += '}';
	} else {
			serializedString += obj; // For primitive types
	}
